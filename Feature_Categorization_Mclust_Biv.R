...
######## CATEGORIAZTION THROUGH MIXTURE MODELLING, BIVARIATE NORMAL ###############

##### CN ####


res <- densityMclust(dati_visit1[, c("CN", "chr")], G=1:15)

result_CN <- mergenormals(dati_visit1[, c("CN", "chr")], res , method="demp", cutoff = 0.2)
summary(result_CN)

dati_visit1$CN_codeM <- res$classification
...
ggplot(dati_visit1) +  # plot density of general CN and mxture components CN
  ...
  geom_density(aes(x=CN, y=..count..), lwd=1.1) + xlim(0,6) +scale_fill_viridis_d() + facet_wrap(~chr)
